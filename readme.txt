...
* Mobile: the portal's data tables (documents, signers, contacts, team) now
  collapse into readable, labelled cards on small screens instead of cramped
  horizontal tables — each value shows its column label, and actions stay easy
  to reach. RTL-aware.
...
* New: a public pricing table shortcode, [comsign_pricing], for marketing pages.
  It shows all plans (price, limits, features) with a "start free trial" call
  to action that sends visitors to register/sign in and lands them in a new
  workspace with their trial. Self-contained styles, RTL-aware.
...
* New: a guided dashboard. New workspaces get a friendly empty state that walks
  the user to creating their first document; the dashboard shows a clear
  "New document" action and human status labels. Owners see a free-trial banner
  with days remaining and a one-click path to choose a plan.
